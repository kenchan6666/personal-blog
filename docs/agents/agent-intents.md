...
| 写履历栏目 | `portfolio_update_resume` | 单栏只改点名的字段；「更新一下我的 CV」走 skill 整页：分析现有履历并写回 summary、经历和全部项目要点，再 `portfolio_generate_resume` |
| 从 GitHub 加履历项目 | `portfolio_add_resume_project_from_github` | 自动填名称、短技术栈和 XYZ 要点；日期留给 Owner |
