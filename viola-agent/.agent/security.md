...
Search and notebook tools resolve paths through `_resolve_path` (`agent/tools/workspace_paths.py`), which enforces that the resolved path must lie under `allowed_dir` (typically the configured workspace), plus the media upload directory (`get_media_dir()`) and any `extra_allowed_dirs`. The interactive agent does not expose generic workspace write tools; Dream consolidation uses internal helpers in `dream_workspace_tools.py`.
