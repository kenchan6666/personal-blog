...
def clear_goal_state(metadata: MutableMapping[str, Any]) -> None:
    """Remove sustained-goal bookkeeping from session metadata."""
    metadata.pop(GOAL_STATE_KEY, None)
...
def runner_wall_llm_timeout_s(
    ...
    *,
    ...
    """Wall-clock cap for :class:`~viola.agent.runner.AgentRunner` when streaming an LLM.

    Returns ``0.0`` to disable ``asyncio.wait_for`` around the request when a sustained goal is
    active; ``None`` means use the default LLM timeout. Pass in-memory ``metadata`` when the
    caller already holds :attr:`~viola.session.manager.Session.metadata` for this turn.
    """
