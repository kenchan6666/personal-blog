...
from viola.agent.runner import AgentRunner, AgentRunSpec
from viola.agent.tools.registry import ToolRegistry
from viola.session.manager import Session
from viola.utils.gitstore import GitStore
...
class Dream:
    ...
    Phase 2 delegates to AgentRunner with read_file-only context tools.
    """
    ...
    def __init__(
        ...
        persistence_via_mcp_only: bool = False,
    ):
        self.store = store
        self.provider = provider
        self.model = model
        ...
        self.persistence_via_mcp_only = persistence_via_mcp_only
    ...
    def _build_tools(self) -> ToolRegistry:
        ...
        from viola.agent.tools.file_state import FileStates
        from viola.agent.tools.dream_workspace_tools import ReadFileTool
    ...
    async def run(self) -> bool:
        ...
        if self.persistence_via_mcp_only:
            new_cursor = batch[-1]["cursor"]
            logger.info(
                "Dream skipped (tools.persistenceViaMcpOnly): advancing cursor {}→{} without MEMORY/SOUL edits",
                last_cursor,
                new_cursor,
            )
        ...
        # Phase 2: Delegate to AgentRunner with read_file-only tools
