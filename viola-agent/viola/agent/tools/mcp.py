"""MCP client: connects to MCP servers and wraps their tools as native viola tools."""
...
_MUTATING_MCP_TOOL_RE = re.compile(
    r"(?:^|_)(create|update|delete|cancel|retry|trigger|transition)$",
    re.IGNORECASE,
)
_SESSION_LOCKS: WeakKeyDictionary[Any, asyncio.Lock] = WeakKeyDictionary()


def mcp_session_lock(session: Any) -> asyncio.Lock:
    existing = getattr(session, "_viola_stdio_lock", None)
    if isinstance(existing, asyncio.Lock):
        return existing
    try:
        lock = _SESSION_LOCKS[session]
        ...
        with suppress(Exception):
            session._viola_stdio_lock = lock
    return lock
...
def _is_mutating_mcp_tool(raw_tool_name: str) -> bool:
    return bool(_MUTATING_MCP_TOOL_RE.search((raw_tool_name or "").strip()))
...
class MCPToolWrapper(Tool):
    """Wraps a single MCP server tool as a viola Tool."""
    ...
    async def execute(self, **kwargs: Any) -> str:
        ...
        async with mcp_session_lock(self._session):
...
class MCPResourceWrapper(Tool):
    """Wraps an MCP resource URI as a read-only viola Tool."""
...
class MCPPromptWrapper(Tool):
    """Wraps an MCP prompt as a read-only viola Tool."""
...
async def connect_mcp_servers(mcp_servers: dict, registry: ToolRegistry) -> dict[str, AsyncExitStack]:
    ...
    async def connect_single_server(name: str, cfg) -> tuple[str, AsyncExitStack | None]:
        ...
        try:
            ...
            registered_count = 0
            registered_tool_wrappers: list[str] = []
            ...
            for tool_def in tools.tools:
                ...
                if (
                    ...
                    continue
                if _customer_service_only_enabled() and _is_mutating_mcp_tool(tool_def.name):
                    logger.info(
                        "MCP: skipping mutating tool '{}' from server '{}' in customer-service-only mode",
                        wrapped_name,
                        name,
                    )
                    continue
                ...
                registered_tool_wrappers.append(wrapper.name)
                registered_count += 1
                if enabled_tools:
            ...
            logger.info(
                "MCP server '{}': connected, {} capabilities registered; tools: {}",
                name,
                registered_count,
                ", ".join(registered_tool_wrappers) if registered_tool_wrappers else "(none)",
            )
...
def runtime_lines(
    ...
    *,
    ...
    if isinstance(metadata, Mapping):
        knowledge_raw = metadata.get("backend_knowledge_context")
    if isinstance(knowledge_raw, Mapping):
        facts = knowledge_raw.get("key_facts")
        policies = knowledge_raw.get("prompt_policies")
        fact_count = len(facts) if isinstance(facts, list) else 0
        policy_count = len(policies) if isinstance(policies, list) else 0
        lines.append(
            "Backend Knowledge Context: "
            f"{fact_count} facts, {policy_count} prompt policies attached. "
            "Prefer these constraints when answering."
        )
    ...
    for item in structured[:8]:
        ...
        if configured_server_names is not None and raw_name not in configured_server_names:
            lines.append(
                ...
                "tell the user to restart viola."
            )
            continue
        if connected_server_names is not None and raw_name not in connected_server_names:
            lines.append(
                ...
                "run the preset test, and restart viola only if hot reload is unavailable."
            )
            continue
        lines.append(
            "MCP Preset Attachment: "
...
async def connect_missing_servers(state: Any, tools: ToolRegistry) -> None:
    """Connect only MCP servers that are configured but not currently connected."""
    configured = getattr(state, "_mcp_servers", {}) or {}
    connected = getattr(state, "_mcp_stacks", {}) or {}
    missing_names = [name for name in configured if name not in connected]
    if not missing_names:
        return
    missing = {name: configured[name] for name in missing_names}
    newly_connected = await connect_mcp_servers(missing, tools)
    if not hasattr(state, "_mcp_stacks") or state._mcp_stacks is None:
        state._mcp_stacks = {}
    state._mcp_stacks.update(newly_connected)


async def handle_runtime_control(state: Any, msg: Any, tools: ToolRegistry) -> bool:
    """Handle simple MCP runtime commands from user messages."""
    raw = str(getattr(msg, "content", "") or "").strip().lower()
    if raw not in {"/mcp reconnect", "/mcp reload", "/mcp refresh", "/mcp status"}:
        return False

    configured = sorted((getattr(state, "_mcp_servers", {}) or {}).keys())
    connected = sorted((getattr(state, "_mcp_stacks", {}) or {}).keys())
    failed = [name for name in configured if name not in connected]

    if raw in {"/mcp reconnect", "/mcp reload", "/mcp refresh"}:
        await connect_missing_servers(state, tools)
        connected = sorted((getattr(state, "_mcp_stacks", {}) or {}).keys())
        failed = [name for name in configured if name not in connected]

    if not hasattr(state, "bus"):
        return True
