...
from viola.agent.tools.base import Tool, tool_parameters
...
from viola.cron.types import CronJob, CronJobState, CronSchedule
...
class CronTool(Tool, ContextAware):
    ...
    def __init__(
        self,
        cron_service: CronService | CronRegistry,
        default_timezone: str = "UTC",
        allow_api_scheduling: bool = False,
    ):
        self._cron_backend = cron_service
        self._default_timezone = default_timezone
        self._allow_api_scheduling = allow_api_scheduling
    ...
    def create(cls, ctx: Any) -> Tool:
        allow_api = getattr(getattr(ctx, "config", None), "allow_scheduled_tasks_on_api", False)
        return cls(
            cron_service=ctx.cron_service,
            default_timezone=ctx.timezone,
            allow_api_scheduling=allow_api,
        )
    ...
    async def _active_cron(self) -> CronService:
        backend = self._cron_backend
        if isinstance(backend, CronRegistry):
            key = self._session_key.get() or ""
            svc = backend.for_session_key(key)
            await backend.ensure_started(svc)
            return svc
        return backend
    ...
    async def execute(
        ...
        **kwargs: Any,
        ...
        if self._channel.get() == "api" and not self._allow_api_scheduling:
            return (
                "Error: scheduled reminders and periodic tasks are not available on "
                "this WhatsApp-linked channel. Reply to the user in natural language "
                "that 「提醒／定時任務」functions are temporarily unavailable — do not retry cron."
            )
        if action == "add":
            if self._in_cron_context.get():
                return "Error: cannot schedule new jobs from within a cron job execution"
            cron = await self._active_cron()
            return self._add_job(
                cron, name, message, every_seconds, cron_expr, tz, at, deliver,
            )
        elif action == "list":
            cron = await self._active_cron()
            ...
            cron = await self._active_cron()
            return self._remove_job(cron, job_id)
    ...
    def _add_job(
        ...
        channel_meta = dict(self._metadata.get())
        if channel == "api":
            recipient = whatsapp_recipient_from_session_key(session_key)
            if recipient:
                channel_meta["whatsapp_recipient"] = recipient
            else:
                return "Error: no WhatsApp session (schedule reminders from an active chat first)"
    ...
    def _remove_job(self, cron: CronService, job_id: str | None) -> str:
        if not job_id:
