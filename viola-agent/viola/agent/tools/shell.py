...
from viola.agent.tools.exec_session import (
...
from viola.agent.tools.sandbox import wrap_command
from viola.agent.tools.schema import IntegerSchema, StringSchema, tool_parameters_schema
from viola.config.paths import get_media_dir
from viola.config.schema import Base
...
class ExecTool(Tool):
    ...
    def enabled(cls, ctx: Any) -> bool:
        if not ctx.config.exec.enable:
            return False
        # Shell is a common bypass for HTTP clients; durable data must use MCP when enabled.
        if getattr(ctx.config, "persistence_via_mcp_only", False):
            return False
        return True
    ...
    def __init__(
        ...
        self.deny_patterns = (deny_patterns or []) + [
            ...
            r":\(\)\s*\{.*\};\s*:",          # fork bomb
            # Block writes to viola internal state files (#2989).
    ...
    def description(self) -> str:
        return (
            ...
            "Prefer MCP/API-backed tools for reading or writing project data; use grep/glob for local code search "
            "over shell find/grep. "
    ...
    async def execute(
        ...
        if self.path_append:
            if _IS_WINDOWS:
                ...
                env["VIOLA_PATH_APPEND"] = self.path_append
                command = f'export PATH="$PATH{os.pathsep}$VIOLA_PATH_APPEND"; {command}'
    ...
    def _guard_command(self, command: str, cwd: str) -> str | None:
        ...
        from viola.security.network import contains_internal_url
