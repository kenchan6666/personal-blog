...
from viola.agent.hook import AgentHook, AgentHookContext
...
_COMPACTABLE_TOOLS = frozenset({
    "exec", "grep", "glob",
    "web_search", "web_fetch", "list_exec_sessions"
})
...
class AgentRunSpec:
    ...
    # Tool names withheld from definitions sent to the LLM this run (still registered).
    excluded_prompt_tools: frozenset[str] = frozenset()
...
class AgentRunner:
    ...
    async def _request_model(
        ...
        if timeout_s is None:
            ...
            # Set VIOLA_LLM_TIMEOUT_S=0 to disable.
            raw = os.environ.get("VIOLA_LLM_TIMEOUT_S", "300").strip()
            try:
                timeout_s = float(raw)
        ...
        kwargs = self._build_request_kwargs(
            spec,
            messages,
            tools=spec.tools.get_definitions(spec.excluded_prompt_tools or None),
        )
    ...
    def _snip_history(
        ...
        estimate, _ = estimate_prompt_tokens_chain(
            ...
            spec.model,
            messages,
            spec.tools.get_definitions(spec.excluded_prompt_tools or None),
        )
        if estimate <= budget:
            return messages
