...
- 履历：`portfolio_list_resume_templates` / `get_resume_template` / `create_resume_template` / `update_resume_template`；`portfolio_list_resumes` / `get_resume` / `create_resume` / `update_resume`；`portfolio_generate_resume` 用 Python 按模板出 PDF；`portfolio_publish_resume` 仅在 Owner 明确要求发布时调用；`portfolio_import_resume_from_github` 从已授权仓库读取 JSON（含抽出的 format.json）；`portfolio_ensure_cv_repo` 确保私有 `cv` 仓；`portfolio_push_resume_to_github` 把 JSON+PDF 写入该仓。写完同步 RAG。
