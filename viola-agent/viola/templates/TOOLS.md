...
- 履历：写栏目先读 `skills/write-resume/SKILL.md`，再 `portfolio_update_resume`（header/summary/education/internships/workExperiences/projects/activities/skills/languages/extras）。栏目要点必须是数组，一行一条。从 GitHub 加项目用 `portfolio_add_resume_project_from_github`（名称和技术栈、功能要点自动填，日期留给 Owner）。版式用 `portfolio_list_resume_templates` 后换 `templateSlug`。只有 classic-a4 锁在代码里；其余是 `cv` 仓 `template/{slug}.json`，改名/栏目走 `portfolio_update_resume_template`，新建也会写入该文件夹。`portfolio_generate_resume` 出 PDF；`portfolio_publish_resume` 仅在 Owner 明确要求发布时调用；取消发布用 `portfolio_unpublish_resume`，公开页随即消失。`portfolio_push_resume_to_github` 写入私有 `cv` 仓。写完同步 RAG。
