...
- 改首页 / Hero / 简介 / 技能 / 经历条：`portfolio_get_site` 再 `portfolio_update_site`。不要找名叫 main 的 About 页。
- 改 About 页：`portfolio_list_content` kind=`about`，用返回的 id 或 slug（模块 kind 为 summary / education / experience / achievement / custom）。
...
- `mcp_portfolio_portfolio_update_content`：只传用户要求改变的字段；About 用 kind=`about`。
...
- `mcp_portfolio_portfolio_list_knowledge`：查看“关于我”RAG 资料（私有记忆，不是首页）。
