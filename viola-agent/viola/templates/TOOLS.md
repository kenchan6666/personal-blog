...
- `mcp_portfolio_portfolio_get_github_source` / `get_github_file`：用 `owner/name` 或唯一仓库短名（如 `taiko_bot_qq`）读取已授权仓库。README 文件名大小写不敏感。需要时直接调用，不要只预告。读完只摘要，不要把整份文件贴回对话。
