...
def looks_like_html_response(text: str | None) -> bool:
    """True when *text* looks like an HTML page (e.g. upstream API mis-route)."""
    if not text:
        return False
    head = text.lstrip()[:512].lower()
    return head.startswith("<!doctype") or head.startswith("<html") or "<html" in head[:256]
...
_TOOL_RESULTS_DIR = ".viola/tool-results"
