...
class ProviderSpec:
    ...
    display_name: str = ""  # shown in `viola status`

    # which provider implementation to use
    # "openai_compat" | "anthropic" | "azure_openai" | "bedrock"
    ...
    # per-model api_base overrides (keyword matched against model name), e.g.
    # (("claude", "https://api.uniapi.io/claude"),)
    model_api_bases: tuple[tuple[str, str], ...] = ()
...
def normalize_uniapi_api_base(raw: str) -> str:
    """Return the OpenAI-compatible UniAPI base URL.

    Common misconfigurations:
    - ``https://uniapi.ai`` (marketing portal) → API gateway
    - legacy regional or Anthropic-native URLs → the configured OpenAI-compatible gateway
    """
    ...
    if "uniapi.ai" in lower and "api.uniapi.io" not in lower:
        logger.warning(
            ...
            raw,
            _UNIAPI_OPENAI_COMPAT_BASE,
        )
        return _UNIAPI_OPENAI_COMPAT_BASE
    if lower.endswith("/claude"):
        logger.warning(
            "Normalizing UniAPI api_base {!r} to {} (OpenAI-compatible chat uses /v1)",
            raw,
            _UNIAPI_OPENAI_COMPAT_BASE,
        )
        return _UNIAPI_OPENAI_COMPAT_BASE
    if "hk.uniapi.io" in lower:
        logger.warning(
            "Normalizing UniAPI api_base {!r} to {}",
            raw,
            _UNIAPI_OPENAI_COMPAT_BASE,
        )
        return _UNIAPI_OPENAI_COMPAT_BASE
