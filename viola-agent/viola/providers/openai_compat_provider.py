"""OpenAI-compatible provider focused on OpenRouter and generic chat completions."""
...
_DEFAULT_OPENROUTER_HEADERS = {
    "HTTP-Referer": "https://github.com/virpluzteam/curtain-lolanto.git",
    "X-OpenRouter-Title": "viola-agent",
    "X-OpenRouter-Categories": "viola-agent",
}
...
def _is_kimi_thinking_model(model_name: str) -> bool:
    ...
    Supports bare slugs and OpenRouter-style ``"publisher/slug"`` names.
    """
...
def _short_tool_id() -> str:
    """9-char alphanumeric ID compatible with OpenAI-compatible APIs."""
...
def _uses_openrouter_attribution(spec: "ProviderSpec | None", api_base: str | None) -> bool:
    """Apply Viola attribution headers to OpenRouter requests by default."""
...
class OpenAICompatProvider(LLMProvider):
    """OpenRouter-oriented OpenAI-compatible chat (Chat Completions API).
    ...
    def _build_kwargs(
        ...
        # Backfill reasoning_content="" on assistant turns when extended thinking
        # is enabled so upstream models that expect the field do not error.
        explicit_thinking = (
    ...
    def _extract_usage(cls, response: Any) -> dict[str, int]:
        ...
        responses.  Different gateways expose cached prompt tokens under
        different field layouts; the priority chain picks the first match.
        """
    ...
    @classmethod
    def _finalize_response(cls, response: LLMResponse) -> LLMResponse:
        if looks_like_html_response(response.content):
            preview = (response.content or "")[:120].replace("\n", " ")
            logger.error("Upstream returned HTML instead of model text: {!r}", preview)
            return LLMResponse(
                content=(
    ...
    def _parse(self, response: Any) -> LLMResponse:
        if isinstance(response, str):
            return self._finalize_response(LLMResponse(content=response, finish_reason="stop"))
        ...
        if response_map is not None:
            ...
            # When the assistant message has no content, some gateways put the
            # visible text in a separate "reasoning" field instead.
