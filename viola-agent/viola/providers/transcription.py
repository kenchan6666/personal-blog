"""Voice transcription providers (Groq, OpenAI Whisper, UniAPI STT)."""
...
def _input_audio_format(path: Path) -> str:
    """Map file to OpenAI-compatible ``input_audio.format`` (wav, mp3, ogg, …)."""
...
async def _post_gateway_stt_with_retry(
    *,
    api_url: str,
    api_key: str,
    path: Path,
...
async def _gateway_chat_input_audio_transcribe(
    *,
    api_base: str,
    api_key: str,
    path: Path,
    ...
    """Transcribe via ``/chat/completions`` with ``input_audio`` (Gemini, etc.)."""
    try:
        raw = path.read_bytes()
    except OSError as e:
...
class UniAPITranscriptionProvider:
    """Speech-to-text via UniAPI ``/audio/transcriptions`` (e.g. Gemini)."""
    ...
    def __init__(
        ...
        self._api_base = (
            api_base
            or os.environ.get("VIOLA_PROVIDERS__UNIAPI__API_BASE")
            or _UNIAPI_DEFAULT_API_BASE
        ).rstrip("/")
    ...
    async def transcribe(self, file_path: str | Path) -> str:
        if not self.api_key:
            logger.warning("UniAPI API key not configured for transcription")
            return ""
        path = Path(file_path)
        if not path.exists():
            logger.error("Audio file not found: {}", file_path)
            return ""
        via_stt = await _post_gateway_stt_with_retry(
            api_url=self.api_url,
            api_key=self.api_key,
            path=path,
            model=self.model,
            language=self.language,
            provider_label="UniAPI",
        )
        if via_stt.strip():
            return via_stt

        logger.info(
            "UniAPI dedicated STT returned empty; trying chat/completions input_audio model={}",
            self.model,
        )
        return await _gateway_chat_input_audio_transcribe(
