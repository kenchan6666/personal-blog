...
from viola import __version__
from viola.bus.events import OutboundMessage
from viola.command.router import CommandContext, CommandRouter
from viola.session.goal_state import goal_state_ws_blob
from viola.utils.helpers import build_status_content
from viola.utils.restart import set_restart_notice_to_env
...
NEW_SESSION_COMMANDS: frozenset[str] = frozenset({
    "/new",
    "/reset",
    "/clear",
    "start fresh",
...
def is_new_session_command(raw: str) -> bool:
    """Return True when *raw* starts a fresh conversation for this chat."""
    return raw.strip().lower() in NEW_SESSION_COMMANDS
...
async def cmd_restart(ctx: CommandContext) -> OutboundMessage:
    ...
    async def _do_restart():
        ...
        os.execv(sys.executable, [sys.executable, "-m", "viola"] + sys.argv[1:])
...
async def cmd_status(ctx: CommandContext) -> OutboundMessage:
    ...
    loop = ctx.loop
    runtime = loop.runtime_for_session_key(ctx.key)
    session = ctx.session or runtime.sessions.get_or_create(ctx.key)
    ctx_est = 0
    with suppress(Exception):
        ctx_est, _ = runtime.consolidator.estimate_session_prompt_tokens(session)
    if ctx_est <= 0:
    ...
    with suppress(Exception):
        from viola.utils.searchusage import fetch_search_usage
...
async def _publish_inactive_goal_state(loop, msg) -> None:
    """Notify WebSocket clients that any sustained goal was cleared."""
...
async def cmd_new(ctx: CommandContext) -> OutboundMessage:
    ...
    runtime.auto_compact._summaries.pop(ctx.key, None)
    runtime.auto_compact._archiving.discard(ctx.key)
    if snapshot:
        loop._schedule_background(runtime.consolidator.archive(snapshot))
    await _publish_inactive_goal_state(loop, ctx.msg)
    return OutboundMessage(
...
async def cmd_dream(ctx: CommandContext) -> OutboundMessage:
    ...
    async def _run_dream():
        t0 = time.monotonic()
        try:
            did_work = await runtime.dream.run()
...
async def cmd_dream_log(ctx: CommandContext) -> OutboundMessage:
    ...
    """
    runtime = ctx.loop.runtime_for_session_key(ctx.key)
    store = runtime.consolidator.store
    git = store.git
...
def register_builtin_commands(router: CommandRouter) -> None:
    ...
    for command in sorted(NEW_SESSION_COMMANDS):
        router.exact(command, cmd_new)
