---
name: update-setup
description: One-time setup wizard for the viola upgrade skill. Triggers: setup update, configure update, 切设置更新, 初始化更新.
---
...
Use `exec` to run `viola --version`. Tell the user the current version.

Then collect install clues with `exec`. These commands are best-effort; if one fails,
keep going and show the useful output:

```
command -v viola || true
python -m pip show viola-ai || true
pipx list | sed -n '/viola-ai/,+3p' || true
uv tool list | sed -n '/viola-ai/,+3p' || true
```
...
answer, stop and ask the user to rerun this setup when they know how viola was
installed.
...
`question: "Where is your viola source checkout? Enter an absolute path or a path relative to this workspace:"`.
...
| Method | Command |
|--------|---------|
| uv | `uv tool install "viola-ai[EXTRAS]" --force` |
| pipx | `pipx install --force "viola-ai[EXTRAS]"` |
| pip | `python -m pip install --upgrade "viola-ai[EXTRAS]"` |
...
If a writable file tool is available in this runtime, create `skills/update/SKILL.md` with this content. Otherwise, output the full content and ask the user to save it manually:

```
---
name: update
description: "Upgrade viola to the latest version. Triggers: upgrade viola, update viola, 升级viola, 更新viola."
---
...
4. Use `exec` to verify: `viola --version`
5. Tell the user the new version. Say: "Run `/restart` to restart viola and apply the update. If `/restart` is unavailable in this channel, restart the viola process manually."
```

## Step 5: Confirm

Tell the user: "Upgrade skill created. Say 'upgrade viola' when you want to update."
