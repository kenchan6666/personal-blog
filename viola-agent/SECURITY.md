...
If you discover a security vulnerability in viola, please report it by:
...
chmod 600 ~/.viola/config.json

# ❌ Bad: Hardcoding keys in code or committing them
```

**Recommendations:**
- Store API keys in `~/.viola/config.json` with file permissions set to `0600`
...
- Config files and API keys (`~/.viola/config.json`) → **hidden** (masked by tmpfs)
...
- ✅ Run viola with a dedicated user account
...
- Keep authentication data in `~/.viola/whatsapp-auth` secure (mode 0700)
...
- Subscribe to security advisories for viola and its dependencies
...
1. **Isolate the Environment**
   ...
   pip install viola-ai
   ```

2. **Use a Dedicated User**
   ```bash
   sudo useradd -m -s /bin/bash viola
   sudo -u viola viola gateway
   ```
...
3. **Set Proper Permissions**
   ...
   chmod 700 ~/.viola
   chmod 600 ~/.viola/config.json
   chmod 700 ~/.viola/whatsapp-auth
   ```

4. **Enable Logging**
   ```bash
   # Configure log monitoring
   tail -f ~/.viola/logs/viola.log
   ```
...
- **Chat history is stored locally** - protect the `~/.viola` directory
...
2. **Review logs for unauthorized access**
   ```bash
   grep "Access denied" ~/.viola/logs/viola.log
   ```
